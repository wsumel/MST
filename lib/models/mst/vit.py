...
from lib.models.mst.base_backbone import BaseBackbone
...
def _create_vision_transformer(variant, pretrained=True,tiny=True, default_cfg=None, **kwargs):
    ...
    if pretrained:
        if tiny:
            print("tiny")
            checkpoint = torch.load('/disk3/wsl/disk3/wsl_tmp/mst-tiny/pretrain/mae_tiny_distill_400e.pth.tar', map_location="cpu")
            new_dict = {}
...
def vit_tiny_patch16_224(pretrained=False,tiny=True, **kwargs):

    model_kwargs = dict(
        patch_size=16, embed_dim=192, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_tiny_patch16_224_in21k',pretrained=True,tiny=True, **model_kwargs)
    return model
